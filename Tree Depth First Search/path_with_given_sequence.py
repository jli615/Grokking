...
def find_path_helper(root, sequence, sequenceIndex):
  # base case
  if root == None:
    return False

  # check if current sequence val is valid
  if sequence[sequenceIndex] != root.val:
    return False

  # leaf case
  if root.left == None and root.right == None and sequenceIndex == len(sequence) - 1:
    return True
  
  # recursive case
  return find_path_helper(root.left, sequence, sequenceIndex + 1) or find_path_helper(root.right, sequence, sequenceIndex + 1)
