...
class MaximumPathSum:
  ...
  def find_maximum_path_sum(self, root):
    self.max_weight = -math.inf
    self.calculate_max_weight(root)
    return self.max_weight

  def calculate_max_weight(self, node):
    # base case
    if node == None:
      return 0

    # find weights of left, right
    left_weight = self.calculate_max_weight(node.left)
    right_weight = self.calculate_max_weight(node.right)

    # max weights
    max_left_weight = max(left_weight, 0)
    max_right_weight = max(right_weight, 0)

    node_weight = left_weight + right_weight + node.val

    # compare to current max
    self.max_weight = max(self.max_weight, node_weight)

    return max(max_left_weight, max_right_weight) + node.val
